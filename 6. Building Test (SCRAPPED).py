...
def main(animCounter, unitTypeList, attackTypeList, buildingTypeList):
    ...
    pygame.display.set_caption("Background Test 1")
    background_image = pygame.image.load(os.path.join('sprites', 'BGTEST.png')).convert()
    imageSize = [1920,1080]
    ...
    TempBuildList = pygame.sprite.OrderedUpdates()

    for object in buildingTypeList:
        if object.Name == "Nexus":
            # create the sprite
    ...
    selectedObject = None
    buildKeyPressed = False #used to determine if b was pressed last
    ...
    while running:
        ...
        for event in pygame.event.get():
                ...
                if event.button == 1: #on left click
                    ...
                    for object in UnitList: #for all sprites
                        if approxEquals(object, event.pos[0], event.pos[1]):
                            ...
                            changed = True
                            buildKeyPressed = False #cancels builder menu
                    for object in BuildingList: #for all buildings too
                        if approxEquals(object, event.pos[0], event.pos[1]):
                            selectedObject = object #set new selected sprite
                            changed = True
                            buildKeyPressed = False #cancel builder menu
                ...
                if event.button == 3:
                    if selectedObject != None:
                        if selectedObject.isBuilding != True:
                            attacked = False
                            for object in UnitList: #for all sprites
                                if attacked == False and approxEquals(object, event.pos[0], event.pos[1]) and object != selectedObject and object.Type.Team != selectedObject.Type.Team:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_b: #keydown b
                    if selectedObject != None and selectedObject.Type.isBuilder == True and buildKeyPressed == False:
                ...
                if event.key == pygame.K_e: #keydown e
                    ...
                    if selectedObject != None and selectedObject.Type.isBuilder and buildKeyPressed == True:
                        #code for building pylon
                        print("a")
                        pos = pygame.mouse.get_pos()
                        for object in buildingTypeList:
                            if object.BuildKey == "e":
                                TempBuilding = Sprite(object)
                                TempBuilding.trueX = pos[0]
                                TempBuilding.trueY = pos[1]
        ...
        for object in BuildingList: #building list collisions
            ...
            for object in collisionList:
                ...
                object.rect.center = (object.trueX - 10, object.trueY - 10) #move out of collision range to allow retargeting
                if pygame.sprite.collide_rect(object, Building): #check move worked
                    object.rect.center = (object.trueX + 10, object.trueY + 10) #move in other direction to compensate
        ...
        for counter in range(0,2):
            if counter == 0:
                ...
                for object in AttackList:
                    if approxEquals(object, object.targetSprite.trueX, object.targetSprite.trueY):
        ...
        for object in BuildingList:
            if object.Type.isResearching == True:
                ...
                if object.researchCounter >= (object.Type.researchType.buildTime * 30):
                    object.Type.researchType.x = object.trueX + (object.Type.size / 2) + 15
                    object.Type.researchType.y = object.trueY + (object.Type.size / 2) + 15#sets spawn point
...
if __name__ == "__main__":
    ...
    pylon.size = 64
    pylon.Name = "Pylon"
    pylon.Image1 = "Pylon 64x64.bmp"
    pylon.Team = "Protoss"
    pylon.BuildKey = "e"
    buildingTypeList.append(pylon)

    nexus = buildingType()
    nexus.size = 256
    nexus.Name = "Nexus"
    nexus.Image1 = "Nexus 256x256.bmp"
    nexus.Team = "Protoss"
    nexus.hotKey = ["e"]
    nexus.unitOrder = ["Probe"]
    nexus.BuildKey = "n"
    buildingTypeList.append(nexus)


    #attack definitions
    particleDisruptors = attackType()
    particleDisruptors.size = 32
    ...
    particleBeam = attackType()
    particleBeam.size = 16
    particleBeam.Name = "Particle Beam"
    particleBeam.Image1 = 'ProbeAttack1 16x16.bmp'
    particleBeam.Image2 = 'ProbeAttack2 16x16.bmp'
    particleBeam.attackDamage = 5
    particleBeam.attackAir = False
    particleBeam.attackGround = True
    particleBeam.attackRange = 0.1
    particleBeam.attackCooldwon = 1.5
    attackTypeList.append(particleBeam)
    ...
    probe = unitType()
    probe.size = 32
    probe.attackType = particleBeam
    probe.Name = "Probe"
    probe.Team = "Protoss"
    probe.Image1 = 'Probe1 32x32.bmp'
    probe.Image2 = 'Probe2 32x32.bmp'
    probe.Speed = 2.8125
    probe.buildTime = 17
    probe.unitHealth = 40
    probe.unitFlying = False
    probe.isBuilder = True
    unitTypeList.append(probe)
