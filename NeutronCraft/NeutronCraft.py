    ...
    while running:
        ...
        if gameOver == 2: #menu event
            ...
            if menu1var == False:
                ...
                tutorial = buildingType()
                tutorial.Image1 = "Tutorial.bmp"
                tutorial.x = 60
                tutorial.y = 25
                tutorial.Name = "Tutorial"
                MenuList.add(Sprite(tutorial))
                rasPi = buildingType()
                        ...
                        for object in MenuList: #for all sprites
                            if approxEquals(object, event.pos[0], event.pos[1]):
                                if object.Type.Name == "Protoss1":
                                    ...
                                    running = False
                                elif object.Type.Name == "Tutorial":
                                    gameOver = 3
            ...
            if enemyTeam != None and gameOver != 3:
                buildvar = False
                selectedObject = None
                endDraw = True
                MenuList.empty()
        ...
        if gameOver == 3: #tutorial event
            ...
            if tutorialProgress == 0: #tutorial phase 1

                if buildvar == False: #make a barracks
                    buildvar = True
                    for object in buildingTypeList:
                        # create the sprite
                        if object.Name == "Barracks":
                            ...
                            break
                    minerals = 0
                    pygame.mixer.Sound(os.path.join('audio',"Tutorial1.wav")).play()
                    tutorialText = font.render("Click on the Barracks!", 1, (0,0,255))
                    tutorialPos = tutorialText.get_rect(center=(600, 580))

                if selectedObject != None and selectedObject.Type.Name == "Barracks":
                    tutorialProgress = 1
                    buildvar = False
                    pygame.mixer.stop()
                    minerals = 50

            elif tutorialProgress == 1: #phase 2
                if buildvar == False: #make a barracks
                    buildvar = True
                    pygame.mixer.Sound(os.path.join('audio',"Tutorial2.wav")).play()
                    tutorialText = font.render("Press ""A"" to create a Marine!", 1, (0,0,255))
                    tutorialPos = tutorialText.get_rect(center=(600, 580))

                for object in UnitList:
                    if object.Type.Name == "Marine":
                        tutorialProgress = 2
                        for object in unitTypeList:
                            if object.Name == "Zealot":
                                object.x = 1000
                                object.y = 300
                ...
                if buildvar == False: #make a barracks
                    buildvar = True
                    pygame.mixer.Sound(os.path.join('audio',"Tutorial3.wav")).play()
                    tutorialText = font.render("Kill the Zealot with your Marine!", 1, (0,0,255))
                    tutorialPos = tutorialText.get_rect(center=(600, 580))
                ...
                if buildvar == False: #make a barracks
                    buildvar = True
                    pygame.mixer.Sound(os.path.join('audio',"Tutorial4.wav")).play()
                    tutorialText = font.render("Note how you earned minerals for killing the zealot!", 1, (0,0,255))
                    tutorialPos = tutorialText.get_rect(center=(490, 580))
                ...
                if pygame.mixer.get_busy() == False:
                    ...
                    playerTeam = None
                    enemyTeam = None
            screen.blit(tutorialText, tutorialPos)
        ...
        if gameOver == 1: #game end event
            if endDraw == True:
                if wave == 10 and enemiesAlive == False:
                    ...
                    end3 = font.render("Press ''Escape'' to return to the Main Menu", 1, (0,220,50))
                    end3Pos = end3.get_rect(center =(400,375))
                    endPos = end.get_rect(center=(400, 275))
                    end2Pos = end2.get_rect(center=(400, 300))
                    screen.blit(end, endPos)
                    screen.blit(end2, end2Pos)
                    screen.blit(end3, end3Pos)
                    award = buildingType()
                    ...
                    endPos = end.get_rect(center=(400, 275))
                    end3 = font.render("Press ''Escape'' to return to the Main Menu", 1, (255,0,0))
                    end3Pos = end3.get_rect(center=(400,375))
                                ...
                                for object in UnitList: #for all sprites
                                    if attacked == False and approxEquals(object, event.pos[0], event.pos[1]) and object != selectedObject and object.Type.Team != selectedObject.Type.Team and (selectedObject.Type.attackType.attackAir == object.Type.unitFlying or selectedObject.Type.attackType.attackGround != object.Type.unitFlying):
                                        ...
                                        if actualDist <= (selectedObject.Type.attackType.attackRange * 50 + (object.Type.size)) and selectedObject.attackCounter >= (selectedObject.Type.attackType.attackCooldown * 30): #range check
                                            ...
                                            attacked = True
                                        break
                                #player no longer needs to attack buidlings, hence commented out
                                ''' 
